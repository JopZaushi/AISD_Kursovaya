...
double Operand_for_one_elem(char operand, double num_one) {
	double result = 0, exp = 2.7183, pi = 3.1416;
	//Unary minus
	if (operand == 126) { result = -1 * num_one; }
	//Root of number
	if (operand == 1) { result = pow(num_one, 0.5); }
	//Sin
	if (operand == 2) { result = sin(num_one * pi / 180); }
	//Cos
	if (operand == 3) { result = cos(num_one * pi / 180); }
	//Logarithm base 2
	if (operand == 4) { result = log2(num_one); }
	//e^(arg)
	if (operand == 5) { result = pow(exp, num_one); }
	//Ctg
	if (operand == 6) { result = (cos(num_one * pi / 180) / sin(num_one * pi / 180)); }
	//Tg
	if (operand == 7) { result = (sin(num_one * pi / 180) / cos(num_one * pi / 180)); }
	////Logarithm base e
	if (operand == 8) { result = log(num_one);; }
	return result;
}
//Operation on two elements
double Operand_for_two_elem(char operand, double num_one, double num_two) {
	double result = 0;
	//Summation
	if (operand == 43) { result = num_one + num_two; }
	//Difference
	if (operand == 44) { result = num_one - num_two; }
	//Multiplication
	if (operand == 46) { result = num_one * num_two; }
	//Division
	if (operand == 47) { result = num_one / num_two; }
	//Expanding a number
	if (operand == 94) { result = pow(num_one, num_two); }
	return result;
}
//Counting and Converting an Expression
void Culc(char operand, String& stack_for_num) {
	char space = ' ';
...
void Postfix_notation_and_Calculation(String& str) {
	...
	char q = 'q';
	bool proverka = true, correct = false;
	for (int i = 0; i < kol_simv; i++) {
		if (correct == false) {
			correct = true;
			//If the symbol is a number
			if ((simv[i] >= 48) && (simv[i] <= 57) || (simv_last == exclamation)) {
				if (((simv_last >= 48) && (simv_last <= 57)) || (simv_last == exclamation)) { //If the psymbol is a number
					str = str + simv[i];
					stack_for_num = stack_for_num + simv[i];//
				}
				else {
					//So that there is no backsliding at the beginning
					if (proverka == true)
						proverka = false;
					else str = str + space;
					str = str + simv[i];
				...
				if ((simv[i] == '.') && ((simv[i + 1] >= 48) && (simv[i + 1] <= 57)) && ((simv_last >= 48) && (simv_last <= 57))) {
					...
					//So that there is no extra space
			...
			if ((simv[i] == '.') && ((simv[i + 1] >= 48) && (simv[i + 1] <= 57)) && ((simv_last >= 48) && (simv_last <= 57))) {
				simv[i] = '!';
				str = str + simv[i]; //Если да , то просто добавляем его, тем самым не разрушая число
				correct = false;
			}
			//Const pi и e
			if ((simv[i] == 'p') && (simv[i + 1] == 'i') && (simv[i + 1] != '\0')) {
				if (proverka == true)
					proverka = false;
				else str = str + space; // В противном  случае добавляем пробел и добавляем символ в главную строку
			...
			//Функции sin,cos,tg,ctg,ln,log,e^x,sqrt
			...
			//If this is the sign of an operation
				...
				if (stack.empty() == false) {
					...
					if ((stack.last_simv(stack) >= simv[i]) || (stack.last_simv(stack) == simv[i] - 1) || (stack.last_simv(stack) == simv[i] + 1)) {
						while ((stack.last_simv(stack) >= simv[i]) || (stack.last_simv(stack) == simv[i] - 1) || (stack.last_simv(stack) == simv[i] + 1)) {
							Culc(stack.last_simv(stack), stack_for_num);
							str = str + space;
	...
	if (correct == true) {
		...
		for (int i = 0; i < str.function_length(); i++) {
			...
			if (str[i] == 3) str[i] = c;
			if (str[i] == 4) str[i] = l;
			if (str[i] == 5) str[i] = exp;
			if (str[i] == 6) str[i] = c;
			if (str[i] == 7) str[i] = t;
			if (str[i] == 8) str[i] = l;
		}
		cout << "The result of the expression: " << stack_for_num << endl;
		cout << "Expression in postfix form: " << str << endl;
	}
}